...
// blocking appointments (no_patient = true) remove overlapping slots.
...
function generateTimeSlots(doctorSlug: string, locationId: string, dateStr: string, dbAvail?: DayAvailability | null): string[] {
  ...
  const availability = dbAvail
    ? dbAvail[dayOfWeek]
    : DEFAULT_WEEK_SLOTS[dayOfWeek as keyof typeof DEFAULT_WEEK_SLOTS];
...
function DoctorBookingContent() {
  ...
  useEffect(() => {
    if (locationId && doctorSlug && doctor && treatment && dbAvailabilityLoaded && blockedDatesLoaded && exceptionalWindowsLoaded) {
  ...
  }, [locationId, doctorSlug, dbAvailability, dbAvailabilityLoaded, doctor, blockedDates, blockedDatesLoaded, exceptionalWindows, exceptionalWindowsLoaded, treatment, treatmentId, categorySlug]);
  ...
  async function checkEmailExists(value: string) {
    ...
    try {
      ...
      const data = await res.json();
      const notFound = !data.exists;
      setEmailNotFoundError(notFound);
      setEmailVerified(!notFound);
    } catch {
