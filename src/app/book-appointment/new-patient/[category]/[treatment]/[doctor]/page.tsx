...
import { getSwissToday, formatSwissYmd, parseSwissDate, getSwissDayOfWeek, formatSwissDateWithWeekday, getSwissDayRange, createSwissDateTime } from "@/lib/swissTimezone";
...
function DoctorBookingContent() {
  ...
  useEffect(() => {
    ...
    const fetchDbAvailability = async () => {
      try {
        ...
        if (data.availability && Object.keys(data.availability).length > 0) {
          ...
          Object.entries(data.availability).forEach(([day, val]) => {
            ...
            if (v.available !== false) {
              avail[Number(day)] = { start: v.start, end: v.end };
            }
          });
    ...
    if (locationId && doctorSlug && !doctorLoading && doctor && treatment && dbAvailabilityLoaded && blockedDatesLoaded) {
      ...
      const findNextAvailableSlots = async () => {
        ...
        try {
          ...
          if (requestSeq !== availabilityRequestSeq.current) return;
  ...
  }, [selectedDate, locationId, doctorSlug, doctor, dbAvailability, availabilityWindow]);
  ...
  return (
    ...
    >
      ...
      <div className="relative max-w-4xl mx-auto px-4 py-8 sm:py-12 sm:px-6 lg:px-8">
        ...
        <Link
          href={`/book-appointment/new-patient/${categorySlug}/${treatmentId}`}
          ...
          <div className="bg-white rounded-xl sm:rounded-2xl shadow-lg p-4 sm:p-6 lg:p-8 border border-slate-200">
            ...
            {step === "info" && (
              <div className="space-y-4">
                ...
                <div>
                  ...
                  {emailVerified && !emailChecking && <p className="text-xs text-green-600 mt-1">✓ {t("booking.emailAvailable")}</p>}
                  {emailExistsError && (
            ...
            {step === "datetime" && (
              <div className="space-y-4">
                        ...
                        return (
                          ...
                          >
                            <span className="block">
                              {new Date(slot.date + "T12:00:00").toLocaleDateString(dateLocale, { timeZone: "Europe/Zurich", weekday: "short", month: "short", day: "numeric" })}
                            </span>
                ...
                ) : nearestAvailableDate === null && !isLoadingDates && availableSlots.length === 0 ? (
                ...
                <div className="bg-slate-50 rounded-xl p-5 space-y-4">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium text-slate-900">
                      {new Date(selectedDate).toLocaleDateString(dateLocale, { weekday: "long", year: "numeric", month: "long", day: "numeric" })}
                    </span>
                  </div>
                  <div className="flex justify-between">
