  ...
  return <div className="flex min-h-[520px] gap-6">
    <aside className="w-96 shrink-0 rounded-2xl border border-slate-200/80 bg-white/80 shadow-sm">
      <div className="flex items-center justify-between border-b border-slate-200/80 px-4 py-3">
        ...
        <button type="button" onClick={() => { setSelectedId("__new__"); setForm(EMPTY_EXCEPTION_FORM); setError(null); }} className="rounded-lg bg-sky-500 px-3 py-1.5 text-xs font-medium text-white hover:bg-sky-600">+ Add</button>
      </div>
      <div className="max-h-[450px] overflow-y-auto">
        ...
        {items.map((item) => <button key={item.id} type="button" onClick={() => select(item)} className={`w-full border-b border-slate-100 px-4 py-3 text-left hover:bg-slate-50 ${selectedId === item.id ? "bg-sky-50" : ""}`}>
          <span className="block truncate text-sm font-medium text-slate-800">{item.booking_doctors?.name || "Unavailable doctor"}</span>
    ...
    <section className="flex-1 rounded-2xl border border-slate-200/80 bg-white/80 shadow-sm">
      {!selectedId ? <div className="flex h-full items-center justify-center px-8 text-center text-sm text-slate-400">Select an exceptional period or add a new one.</div> : <div>
        <div className="border-b border-slate-200/80 px-6 py-4"><h2 className="text-sm font-semibold text-slate-800">{selectedId === "__new__" ? "Add Exceptional Online Availability" : "Edit Exceptional Online Availability"}</h2><p className="mt-1 text-xs text-slate-500">The selected treatments become bookable during this period even when the doctor is normally unavailable. Every other treatment remains closed.</p></div>
        <div className="space-y-5 px-6 py-5">
          ...
          <div><div className="mb-2 flex items-center justify-between"><label className="text-[11px] font-medium uppercase tracking-wide text-slate-400">Allowed treatments</label><span className="text-[11px] text-slate-400">{form.treatmentIds.length} selected</span></div><div className="max-h-56 overflow-y-auto rounded-xl border border-slate-200 p-2">{treatments.map((treatment) => <label key={treatment.id} className="flex cursor-pointer items-center gap-3 rounded-lg px-3 py-2 hover:bg-slate-50"><input type="checkbox" checked={form.treatmentIds.includes(treatment.id)} onChange={() => toggleTreatment(treatment.id)} className="h-4 w-4 rounded border-slate-300 text-sky-500" /><span className="text-sm text-slate-700">{treatment.name}</span></label>)}</div></div>
          <label className="flex items-center gap-2 text-sm text-slate-700"><input type="checkbox" checked={form.enabled} onChange={(event) => setForm((current) => ({ ...current, enabled: event.target.checked }))} className="h-4 w-4 rounded border-slate-300 text-sky-500" />Enabled for online booking</label>
          {error && <p className="rounded-lg bg-red-50 px-3 py-2 text-xs text-red-600">{error}</p>}
        </div>
        <div className="flex justify-between border-t border-slate-200/80 px-6 py-4"><div>{selectedId !== "__new__" && <button type="button" onClick={remove} disabled={saving} className="rounded-lg border border-red-200 px-4 py-2 text-sm text-red-600 hover:bg-red-50 disabled:opacity-50">Delete</button>}</div><button type="button" onClick={save} disabled={saving} className="rounded-lg bg-sky-500 px-5 py-2 text-sm font-medium text-white hover:bg-sky-600 disabled:opacity-50">{saving ? "Saving…" : "Save"}</button></div>
      </div>}
