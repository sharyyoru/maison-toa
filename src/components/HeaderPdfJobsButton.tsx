...
export default function HeaderPdfJobsButton() {
  ...
  const activeCount = pdfCtx.pendingCount;
  const hasFailed = pdfCtx.jobs.some(j => j.status === "failed");
  const badgeCount = activeCount + (hasFailed ? 1 : 0);
      ...
      {dropdownOpen && (
        <div className="absolute right-0 z-50 mt-2 w-80 rounded-xl border border-slate-200 bg-white shadow-lg">
          <div className="flex items-center justify-between border-b border-slate-100 px-4 py-2">
            <h3 className="text-xs font-semibold text-slate-700">PDF & Insurance Queue</h3>
            {activeCount > 0 && (
