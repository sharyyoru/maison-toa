-- ============================================================
-- Laura Bookings Export API - Postgres functions
-- ============================================================
...
BEGIN
  ...
  WITH base AS (
    SELECT
      ...
      a.tracking_params,
      COALESCE(bt.name, s.name, d_s.name, split_part(a.reason, ' [', 1), a.category, '') AS service_name,
    ...
    WHERE a.is_demo = false
      ...
      AND (p_statuses IS NULL OR array_length(p_statuses, 1) IS NULL OR a.status = ANY(p_statuses))
      AND (p_sources IS NULL OR array_length(p_sources, 1) IS NULL OR a.source = ANY(p_sources))
      AND (v_service_patterns IS NULL OR COALESCE(bt.name, s.name, d_s.name, split_part(a.reason, ' [', 1), a.category, '') ILIKE ANY(v_service_patterns))
  )
  SELECT count(*)
  INTO v_total
  FROM base;
...
BEGIN
  ...
  SELECT COALESCE(jsonb_agg(service_name ORDER BY service_name), '[]'::jsonb)
  INTO v_services
  FROM (
    SELECT DISTINCT COALESCE(bt.name, s.name, d_s.name, split_part(a.reason, ' [', 1), a.category, '') AS service_name
    FROM appointments a
    LEFT JOIN booking_treatments bt ON a.booking_treatment_id = bt.id
    LEFT JOIN services s ON (a.service_ids[1] = s.id)
    LEFT JOIN deals d ON a.deal_id = d.id
    ...
    WHERE a.is_demo = false
      AND a.no_patient = false
      AND COALESCE(bt.name, s.name, d_s.name, split_part(a.reason, ' [', 1), a.category, '') <> ''
