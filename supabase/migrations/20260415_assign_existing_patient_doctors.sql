-- ============================================================
-- Migration: Assign doctors to existing-patient treatments
-- per the canonical JSON mapping (2026-04-15).
--
-- Existing patient category UUIDs:
--   Consultation:            dadd20d3-102a-437c-8460-313557ba6732
--   Laser et appareils:      eaf0192f-5132-4f21-8216-83867adb9c03
--   Injections:              ccfa9100-0998-4429-9168-f6e4f6aee61c
--   Dermatologie Esthétique: 7375e326-86c5-402c-9ea0-f4c9aebe8b52
--   Perfusions de vitamine:  676b4739-5786-4ace-b870-b0ee457230ab
--   Offres:                  db9f9c65-a3e1-414a-8b6b-88cfed39017b
--   Soins:                   961e20ca-f390-48c5-98bb-9e0056da2641
-- ============================================================
...
-- ── Step 2: Clear all doctor assignments for existing-patient treatments ──
...
WHERE treatment_id IN (
  ...
  WHERE bc.patient_type = 'existing'
);
...
WITH assignments (treatment_fragment, doctor_slug) AS (VALUES
  ...
  -- (same fragment as consultation variant above — scoped to existing patients)
  ('Acide Hyaluronique, Toxine',                           'sophie-nordback'),
  ('Acide Hyaluronique, Toxine',                           'alexandra-miles'),
  ('Acide Hyaluronique, Toxine',                           'laetitia-guarino'),
  ('Acide Hyaluronique, Toxine',                           'reda-benani'),
...
JOIN booking_categories  bc ON bc.id = bt.category_id AND bc.patient_type = 'existing'
